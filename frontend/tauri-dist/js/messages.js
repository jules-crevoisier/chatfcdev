...
export const formatContent = (raw) => {
  ...
  // Important: on protège le HTML des emojis personnalisés pour éviter
  // que la regex "autolink" transforme aussi les URL à l'intérieur du tag
  // (ex: dans src="http://..."), ce qui casse le rendu.
  s = s.replace(/:([a-z0-9_-]+):/g, (match, ename) => {
    const ce = state.customEmojis.get(ename);
    if (!ce) return match;

    const escName = escHtml(ename);
    emojiBlocks.push(
      `<img src="${escHtml(serverUrl(ce.url))}" class="msg-custom-emoji" alt=":${escName}:" title=":${escName}:" loading="lazy">`,
    );
    return `\x00EMOJI${emojiBlocks.length - 1}\x00`;
  });
  ...
  s = s.replace(/\x00EMOJI(\d+)\x00/g, (_, i) => emojiBlocks[+i] || '&nbsp;');
